...
run_pass_data <-
  defense %>%
  select(name, r_p, gain, cover_call, quarter, down, dist, line, line_type, play_note, action) %>%
...
write_csv(run_pass_data, "full_data.csv")
...
line_chart <-
  run_pass_data %>%
  plot_ly(x = ~name, y = ~gain) %>% add_markers()
